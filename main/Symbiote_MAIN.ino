...
int a = 0;
String humeur[8]  = {"Serein", "Normal_expressif","Timide","Peureux", "Heureux", "Euphorique", "Enervé","Agressif"};
...
int selecthumeur = 0;
int totalSampleNumberInThisCorpus = 0;

//Bounce bouncer ;  // on créer un objet et on alloue un espace mémoire.
//bouncer = new Bounce(BUTTON,5); // réglage / Initialisation de la fonction.
//Bounce bouncer = Bounce( BUTTON,5 ); même chose mais fait en une ligne donc pas besoin de new.
...
void setup() {
  ...
  SPI.setMOSI(7);
  // Serial Peripheral Interface (SPI) is a synchronous serial data protocol used by microcontrollers
  ...
  SPI.setSCK(14);
  //   Synchro d'horloge entre teensy et audio shield.
...
void loop() {
  if (analogRead Proxi < 512 && analogRead Micro < 512) {           // si qu'une seule action, on écrit sans les crochets.
    selecthumeur = 0;                                               
  }
  if (analogRead Proxi < 512 && analogRead Micro > 512) {
    selecthumeur = 1;                                               // 1 = mode humeur euphorique.
  }
  if (analogRead Micro > 512 && analogRead Micro < 512) {
    selecthumeur = 2;
  }
  if (analogRead Micro > 512 && analogRead Micro > 512) {
    selecthumeur = 3;
  ...
  playFile.play("/" + humeur[selecthumeur] + "/" + humeur[selecthumeur] + selectUnplayedSample"); //

//  playFile2.play("/" + humeur[selecthumeur] + "/" + humeur[selecthumeur] + "unplayedSample"); //
}

// fonction de selection d'un sample qui n'as pas encore joué (= URN dans PD ou Max)
// int scanCorpus() {
...
int selectUnplayedSample(totalSampleNumberInThisCorpus) {
  boolean verif = false;
  while (verif == false) {
    verif = true;
